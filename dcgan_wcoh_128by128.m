...
% Load the dataset
datasetFolder = fullfile('D:\PhD Kanan\Freelancing\Ei\Idea 4\Dataset\WCOH');
...
layersGenerator = [
    ...
    transposedConv2dLayer(filterSize,256,'Name','tconv1')
    batchNormalizationLayer('Name','bn1')
    reluLayer('Name','relu1')
    transposedConv2dLayer(filterSize,128,'Stride',2,'Cropping','same','Name','tconv2')
    batchNormalizationLayer('Name','bn2')
    reluLayer('Name','relu2')
    transposedConv2dLayer(filterSize,64,'Stride',2,'Cropping','same','Name','tconv3')
    batchNormalizationLayer('Name','bn3')
    reluLayer('Name','relu3')
    transposedConv2dLayer(filterSize,32,'Stride',2,'Cropping','same','Name','tconv4')
...
layersDiscriminator = [
    ...
    concatenationLayer(3,2,'Name','cat')
    convolution2dLayer(filterSize,3,'Stride',2,'Padding','same','Name','conv1')
    leakyReluLayer(scale,'Name','lrelu1')
    convolution2dLayer(filterSize,64,'Stride',2,'Padding','same','Name','conv2')
    batchNormalizationLayer('Name','bn2')
    leakyReluLayer(scale,'Name','lrelu2')
    convolution2dLayer(filterSize,128,'Stride',2,'Padding','same','Name','conv3')
    batchNormalizationLayer('Name','bn3')
    leakyReluLayer(scale,'Name','lrelu3')
    convolution2dLayer(filterSize,256,'Stride',2,'Padding','same','Name','conv4')
    batchNormalizationLayer('Name','bn4')
    leakyReluLayer(scale,'Name','lrelu4')
    convolution2dLayer(filterSize,512,'Stride',2,'Padding','same','Name','conv5')
...
for epoch = 1:numEpochs
    ...
    while hasdata(augimds)
        ...
        [gradientsGenerator, gradientsDiscriminator, stateGenerator, scoreGenerator, scoreDiscriminator] = ...
            dlfeval(@modelGradients22, dlnetGenerator, dlnetDiscriminator, dlX, dlT, dlZ, flipFactor);
...
%% After training the next part is to generate new images
clc; close all;
numObservationsNew = 1500; % Enter the number of images that you want
idxClass = 3; % This the class.
% 1 = Healthy, 2 = Hyperkinetic Dysphonia
% 3 = Hypokinetic Dysphonia, 4 = Reflux Laryngitis
...
% Write them into folder
num_images = numObservationsNew;
folder = 'D:\PhD Kanan\Freelancing\Ei\Idea 4\Result\Generated Images128x128\OS8\Hypokinetic_Dysphonia';
for i=1:num_images
    img = dlXGenerated(:,:,:,i); % Read the image from the file
    img = extractdata(img); 
    img = gather(img);
    img = double(img);
    baseFileName = sprintf('Hypo%d.jpg',i);
